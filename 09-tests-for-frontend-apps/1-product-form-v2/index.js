...
export default class ProductForm extends Component {
  ...
  async render() {
    const formElement = generateElement(this.#fillTemplate());
  ...
  #renderImages(images) {
    const imageElements = images.map(image => generateElement(this.#fillSortableListItemTemplate(image.url, image.source)))

    const sortableList = new SortableList({ items: imageElements });
  ...
  #appendImageElement(link, name) {
    const imageElement = generateElement(this.#fillSortableListItemTemplate(link, name));
  ...
  #fillSortableListItemTemplate(url, title) {
